...
void ui_event_brightness(lv_event_t * e)
{
    ...
    if(get_backlight_mode() != BACKLIGHT_MODE_MANUAL) {
        return;
    }
    ...
    if(code == LV_EVENT_PRESSED || code == LV_EVENT_PRESSING)
    {
        ...
        if (label_info != NULL) {
            char info_buf[64];
            snprintf(info_buf, sizeof(info_buf), "Manual: Brightness set to %d%%", get_backlight_brightness());
...
void ui_event_imageiconbrightness(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_PRESSED) {
        ...
        backlight_mode_t previous_mode = get_backlight_mode();
        
        // Toggle the mode
        toggle_auto_mode();
        
        // Get the new mode
        backlight_mode_t new_mode = get_backlight_mode();
        ...
        if (previous_mode != new_mode) {
            ...
            int current_brightness = get_backlight_brightness();
...
static void update_brightness_icon(void) {
    ...
    backlight_mode_t current_mode = get_backlight_mode();
...
static void brightness_adjust_timer_callback(void *arg) {
    ...
    if (current_mode == BACKLIGHT_MODE_AUTO_TIME) {
        ...
        int adjusted_brightness = get_backlight_brightness();
    ...
    while (wifi_event_queue != NULL && xQueueReceive(wifi_event_queue, &msg, 0) == pdTRUE) {
        if (display_mutex != NULL && xSemaphoreTake(display_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            ...
            if (msg.event == WIFI_CLOCK_EVENT_TIME_SYNCED && 
                get_backlight_mode() == BACKLIGHT_MODE_AUTO_TIME) {
    ...
    if (event != last_event) {
        switch (event) {
            ...
            case WIFI_CLOCK_EVENT_TIME_SYNCED:
                ...
                if (get_backlight_mode() == BACKLIGHT_MODE_AUTO_TIME) {
...
static void lvgl_task(void *arg) {
    ...
    if (touch_indev != NULL) {
        ESP_LOGI(TAG, "Touch input device found");
    } else {
        ESP_LOGW(TAG, "No touch input device found");
    }

    // Initialize backlight
    backlight_init();
    ESP_LOGI(TAG, "Backlight initialized, current brightness: %d%%", get_backlight_brightness());
    ...
    backlight_mode_t initial_mode = get_backlight_mode();
    ...
    if (ui_brightness != NULL) {
        lv_bar_set_value(ui_brightness, get_backlight_brightness(), LV_ANIM_OFF);
        ESP_LOGI(TAG, "LVGL bar set to %d%%", get_backlight_brightness());
    } else {
    ...
    if (label_info != NULL) {
        char info_buf[64];
        backlight_mode_t mode = get_backlight_mode();
        if (mode == BACKLIGHT_MODE_AUTO_TIME) {
            snprintf(info_buf, sizeof(info_buf), "System Ready - Auto Brightness: %d%%", get_backlight_brightness());
        } else {
            snprintf(info_buf, sizeof(info_buf), "System Ready - Manual Brightness: %d%%", get_backlight_brightness());
        }
        lv_label_set_text(label_info, info_buf);
    }
    ...
    if (initial_mode == BACKLIGHT_MODE_AUTO_TIME) {
        ...
        // Update the UI bar
        if (ui_brightness != NULL) {
            lv_bar_set_value(ui_brightness, get_backlight_brightness(), LV_ANIM_ON);
        }
    ...
    while (1) {
        ...
        backlight_mode_t current_brightness_mode = get_backlight_mode();
